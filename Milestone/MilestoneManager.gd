...
signal unlocked_milestone(milesone: MilestoneResource)
...
#
# Unlocks a milestone.
# This method assumes the player has enough resources.
# You should check this before calling this method, see MilestoneResource::can_unlock() method
#
func unlock_milestone(milestone: MilestoneResource) -> void:
	for resource in milestone.cost.keys():
		SaveManager.save_data.resources[resource] -= milestone.cost[resource]
	add_completed_milestone(milestone)
	unlocked_milestone.emit(milestone)
